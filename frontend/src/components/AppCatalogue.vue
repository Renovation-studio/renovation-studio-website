<template>
  ...
  />
  <SearchBar
    @update:catalogue-items="updateCatalogueItems"
    @reset-catalogue-items="resetCatalogueItems"
  />
  <FilterButtons
    :filters="filters"
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      try {
        const catalogResponse = await axios.get('/api/catalog')
        const catalogueItemsWithDetails = await Promise.all(
          catalogResponse.data.map(async (item) => {
            const detailsResponse = await axios.get(`/api/catalog/${item.element_services_id}`)
      ...
      newFilters.forEach((filter) => {
        if (filter.isActive && filter.isSortable) {
          ...
          if (filter.name === 'А-Я') {
            ...
            catalogueItems.value.sort((a, b) => {
              return filter.sortOrder === 'ascending' ? a.cost - b.cost : b.cost - a.cost
            })
